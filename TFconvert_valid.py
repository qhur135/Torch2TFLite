...
# 모델에 대한 입력값
# image = torch.randn(batch_size, 3, 256, 256, requires_grad=True)
...
# s = time.time()
torch_out = network.torch_model(torch_image) # torch output
# curr_time = (time.time()-s)
# print('torch inference time:')
# print(curr_time)
...
results = model(image)
curr_time = (time.time()-s)
print('tensorflow inference time:')
print(curr_time)
